...
const invoiceFormSchema = z.object({
  ...
  status: z.enum(['DRAFT', 'SENT', 'PARTIALLY_PAID', 'PAID', 'OVERDUE', 'CANCELED']),
...
export function InvoiceForm({ initialData, isEditing = false }: InvoiceFormProps) {
  ...
  const form = useForm<InvoiceFormValues, unknown, InvoiceFormValues>({
    resolver: zodResolver(invoiceFormSchema) as Resolver<InvoiceFormValues, unknown, InvoiceFormValues>,
  ...
  const calculateSummary = useCallback((items: InvoiceItemWithAmount[]) => {
  ...
  }, [initialData, isEditing, calculateSummary]);
  ...
  const onSubmit: SubmitHandler<InvoiceFormValues> = async (data) => {
    try {
      ...
      if (isEditing && initialData) {
        ...
        // Create new invoice
        const newInvoice = await createInvoice(formattedData);
